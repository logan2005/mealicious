  ...
  const handleStatusChange = async (orderId: string, newStatus: Order['status']) => {
    try {
      ...
      if (response.ok) {
        ...
        toast({ title: "Error", description: errorData.error || "Failed to update order status.", variant: "destructive" })
      }
    } catch (error) {
      console.error('Error updating order status:', error)
      toast({ title: "Error", description: "An error occurred while updating order status.", variant: "destructive" })
    }
  }
  ...
  return (
    <div className="min-h-screen bg-gray-50 py-8">
      <div className="max-w-6xl mx-auto px-4">
        <h1 className="text-3xl font-bold text-gray-900 mb-6">Admin Orders Management</h1>
              ...
              <Card key={order.id}>
                ...
                <CardContent>
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
                    <div>
                      <p><strong>User ID:</strong> {order.userId.substring(0, 8)}...</p>

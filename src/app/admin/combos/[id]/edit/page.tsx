  ...
  const fetchCombo = async () => {
    try {
      ...
      if (response.ok) {
        ...
        setComboItems(combo.items.map((item: any) => ({
        ...
        toast({
          title: "Error",
          description: "Failed to fetch combo data",
          variant: "destructive"
        })
      }
    } catch (error) {
    ...
    try {
      ...
      const response = await fetch(`/api/admin/combos/${id}`,
      {
        ...
        body: JSON.stringify({
          ...
          items: comboItems.map(item => ({ productId: item.productId, quantity: item.quantity }))
        })
      })
      ...
      if (response.ok) {
        ...
        toast({
          title: "Error",
          description: error.error || "Failed to update combo",
          variant: "destructive"
        })
      }
    } catch (error) {
    ...
    <div className="min-h-screen bg-gray-50 p-8">
      <div className="max-w-4xl mx-auto">
        <div className="mb-8">
          ...
          <p className="text-gray-600">Update the details of the product combo</p>
        </div>
